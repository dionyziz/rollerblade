...
ledger protocols $\Y[j][][i]$.
Each $\RB[j]$ \emph{simulates} within its implementation $n$ different instances
of the overlay protocol $\Pi$, one for every underlying ledger protocol $\Y[j][][i]$.
% TODO: figure
The \rollerblade client allows the external user to issue a \emph{write} instruction
to any $i^\text{th}$ simulated machine.
