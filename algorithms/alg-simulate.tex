...
\begin{algorithm}
  ...
  \begin{algorithmic}[1]
    \Function{\simulate}{$i, \Ledger[j][i], \simulationRound$}
      \Let{(\writeboxes, \inboxes)}{\this.\prepareSimulationInputs(i, \Ledger[j][i], \simulationRound)}\label{alg.simulate:recursion}

      \Let{\outboxes}{[[\,]]}\label{alg.simulate:outboxes-init}
      \Let{\inboxThisRound}{[\,]}
      \Let{\outboxThisRound}{[\,]}

      \Function{\send}{recp, m}\label{alg.simulate:send}
      ...
      \Let{\Z[j][i]}{\new \Pi^\net(\this.\Delta_u, j, n)}\label{alg.simulate:construct}
