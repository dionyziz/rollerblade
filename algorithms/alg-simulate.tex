  ...
  \begin{algorithmic}[1]
    \Function{\sf simulate}{$\sid, \Pi, i, \Ledger[j][i], \Y[j], \simulationRound, \realityRound$}
      ...
      \Function{\sf send}{recp, m}
        \State{$\textsf{outboxThisRound}.\append((\textsf{recp}, m))$}
      \EndFunction
      ...
      \For{$r \in [\simulationRound]$}
        ...
        \For{$\textsf{data} \in \textsf{writes}[r - 1]$}
          \CommentLine{Submit `write' commands recorded at round $r - 1$}
          \State{$\Z[j][i].\textsf{write}(\textsf{data})$}
        \EndFor

        \For{$(\textsf{from}, m) \in \textsf{inboxes[r - 1]}$}
          \CommentLine{Record network messages disbursed at round $r - 1$}
