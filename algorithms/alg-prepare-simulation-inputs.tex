    ...
    \Function{\prepareSimulationInputs}{$i, \Ledger[j][i], \simulationRound$}
      ...
      \For{$(r, \instruction) \in L$}\label{alg.prepare-simulation-inputs:for}
        ...
        \If{$\instruction\textsf{.type} = \text{`checkpoint'}$}
          ...
          \Let{\ImaginaryLedger[j][i']}{\this.\Y[j][i']\textsf{.untranscribe}(\instruction\textsf{.data.certificate})}\label{alg.prepare-simulation-inputs:untranscribe}
          \Let{\outboxes}{\this.\textsf{simulate}(i', \ImaginaryLedger[j][i'], r - u_i - v_{i'} - 1)\textsf{.outboxes}}\label{alg.prepare-simulation-inputs:recursion}
