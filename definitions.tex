...
instances of $\Pi$ ($\Z[j][1], \ldots, \Z[j][i], \allowbreak \ldots \Z[j][n]$).
...
For example, if $\Y[][1]$ is the Bitcoin protocol, then the $\Y[j][1]$
...
adversary $\mathcal{A}$ (a rushing adversary).
...
execution of $\Pi$. Roughly, a compositor is called \emph{faithful} if these
...
\begin{definition}[Emulation Consistency]
  ...
  is \emph{$(j,H,\Delta_v)$-co\-nsistent},
...
\begin{definition}[Honest View in the Emulated Setting]
  ...
  \end{bmatrix}\,,
  \]
  where $E(H_i, r)$ denotes the return value of invoking,
...
\begin{definition}[Emulated Setting Externalities]
  ...
  $\writeToMachine(i, \data_1),\allowbreak \ldots, \writeToMachine(i, \data_k)$
  all during round $r$.
...
\begin{definition}[Externality Similarity in Distribution]
  ...
  such that $\widetilde{E_1} \distreq E_1$ and $\widetilde{E_2} \distreq E_2$
  and $\widetilde{E_1}(\omega) \laterly{\Delta_u,\Delta_v} \widetilde{E_2}(\omega)$.
\end{definition}
...
\begin{definition}[Replication Faithfulness]\label{def:replication-faithfulness}
  ...
  for all rounds $1 \leq r < R - \Delta_v - \Delta_u$,
  and all compositor party indexes $j, j' \in [m]$ it holds that
  % TODO: notate this as \Sim[j] = \Sim[j'] meaning
  % within the execution \Epsilon of duration R, that for all 1 <= r < R - max
  % it holds that emuSnapshot etc.
  % avoiding the problem of quantifying R(\kappa) (therefore \kappa)
  % before the execution is sampled
  $\Sim[j][r] = \Sim[j'][r]$,
  where $\Sim[j][r]$ (resp. $\Sim[j'][r]$) indicates the result of calling
  \emulationSnapshot of $\LLambda[j]$ (resp. $\LLambda[j']$) with inputs
  $(i, r)$
