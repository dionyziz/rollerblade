...
\begin{lemma}[Rollerblade Consistency]\label{lem:consistency}
  ...
  and let $\Delta_v = \max\{v_i\}_{i \in [n]}$, where $v_i$ denotes the timeliness
...
\begin{proof}
  ...
  When $\emulateMachine$ is invoked after round $r'$ (or $r' + 1$ respectively),
  the value $\this.\now$
  is $r'$, so the check $\simulationRound > \this.\now - v_i$ of Algorithm~\ref{alg.prepare-simulation-inputs}
  ...
  % TODO: Check if past-perfect can be invoked here, in particular see how liveness affects
  % the invocation of the Lemma.
