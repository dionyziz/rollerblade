...
\begin{lemma}[Rollerblade Cross-party]\label{conj:cross-party}
  ...
  $(i, r - v_i - 1, r)$ at the end of round $r$.
\end{lemma}
\begin{proof}[Sketch]
  The function \emulateMachine calls \simulate with overlay party index $i$,
  ledger $\Ledger[j][i][r]$, simulation round $r - v_i - 1$, and current round $r$. It
  suffices to show that all transactions with recorded round
  $< r - v_i$ are the same in $\Ledger[j][i][r]$ and $\Ledger[j'][i][r]$.
  That is because the return value of \simulate only depends on transactions with
  recorded round $< r - v_i$.

  Towards a contradiction, suppose that all transactions with recorded round
  $< r - v_i$ are not the same in $\Ledger[j][i][r]$ and $\Ledger[j'][i][r]$.
  Because $\Ledger[][i]$ is \emph{good}, it is safe, live($u_i$), and timely($v_i$).
  By safety, $\Ledger[j][i][r]$ and $\Ledger[j'][i][r]$
  are consistent. Therefore, without loss of generality, there exists a
  transaction $(r^*, \tx) \in \Ledger[j][i][r]$ with $r^* < r - v_i$
  such that $(r^*, \tx) \not\in \Ledger[j'][i][r]$.

  Due to liveness, ledger $\Ledger[j'][i]$ must grow. Therefore, because of
  safety, transaction $(r^*, \tx')$ must be included in $\Ledger[j'][i][r']$
  (where $r' > r$), at position $|\Ledger[j'][i][r]|$ or later.
  % TODO: this applies only if there are honest transactions every round.
  % TODO: calculate r' wrt u_i
  However, this contradicts timeliness because there exists a transaction
  in $\Ledger[j'][i][r'][|\Ledger[j'][i][r]|{:}]$ with recorded round
  % TODO: This is correct, but maybe we can save one round here?
  % since the result is that r^* < r' - v_i - 1. Maybe the construction
  % can be modified to accommodate for this.
  $r^* < r - v_i < r' - v_i$.\qed
\end{proof}
