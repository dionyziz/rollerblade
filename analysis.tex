...
\begin{lemma}[In Vitro Independence]\label{lem:in-vitro-independence}
  Consider an $(j, H, \Delta_v)$-consistent emulated setting
  execution $\mathcal{E}^*$ in which
  $\Y[i]$ is sticky.
  Consider the configuration $c(j, i, r_1, r_2, r_3)$
  of the machine $\Z[j][i]$ after the invocation of
  Algorithm~\ref{alg.simulate} Line~\ref{alg.simulate:execute}
  during the $r_1$-st iteration of the \emph{for loop} in
  Algorithm~\ref{alg.simulate} Line~\ref{alg.simulate:for}
  when $\simulate$ is
  invoked \emph{in vitro} after round $r_2 \geq r_1 + \Delta_v + 1$ with parameter
  \simulationRound $r_3$, where $r_3 \geq r_1$.
  Then the value of $c$ does not depend on $r_2$ or $r_3$.
\end{lemma}
\begin{proof}
  We observe that the value of $c$ does not
  depend on the \emph{in vitro} round $r_2$ because $\mathcal{E}^*$ is consistent.

  Let $(\writeboxes^{r_3-1}, \inboxes^{r_3-1})$ be the pair
  returned by the invocation of $\prepareSimulationInputs(i, r_3 - 1, r_2)$
  \emph{in vitro} after round $r_2$, and, likewise define
  $(\writeboxes^{r_3}, \inboxes^{r_3})$ to be the pair returned by the invocation
  of $\prepareSimulationInputs(i, r_3, r_2)$ \emph{in vitro} after round $r_2$.
  Because $\Y[j][i]$ is sticky, therefore $\Ledger[j][i][r_3 - 1] \preceq \Ledger[j][i][r_3]$.
  Hence, $\writeboxes^{r_3 - 1} \preceq \writeboxes^r$ and $\inboxes^{r_3 - 1} \preceq \inboxes^{r_3}$
  because the loop of Algorithm~\ref{alg.prepare-simulation-inputs} Line~\ref{alg.prepare-simulation-inputs:for}
  will run identically in the two invocations of $\prepareSimulationInputs$ up to
  and including round $r_3 - 1$.
  The state of the machine at each $(r_3 - 1)$-st iteration of the
  \emph{for} loop only depends on the writeboxes and inboxes at position $(r_3 - 1)$ and behind,
  namely $\writeboxes^{r_3 - 1}[{:}{r_3 - 1}], \inboxes^{r_3 - 1}[{:}{r_3 - 1}]$
  and $\writeboxes^{r_3}[{:}{r_3 - 1}], \inboxes^r[{:}{r_3 - 1}]$ respectively.
  Therefore, the value of $c$ does not depend on $r_3$.
  \qed
\end{proof}
...
\begin{proof}[Sketch]
  ...
  Let $\PPi[i]$ be the $i$-th party in party setting execution $\mathcal{E}'$.
  First, we must show that the environment $\mathcal{Z}'$ respects the delay $\Delta$.
  Namely, for all $i, i'$, for all rounds $r$ of $\mathcal{E}'$:
  ...
  \end{enumerate}
  {\color{red} TODO: prove this.}

  % TODO: Double check this proof, it is a bit convoluted...
  \noindent
  ...
  Let
  $i$ be an arbitrary party in $H$ and $r$ be an arbitrary round $1 \leq r \leq R - \Delta_v - 1$.
  We will prove that $V^*_{i,r} = V'_{i,r}$ by induction on the round $r$.

  Induction base: {\color{red} Finish the base...}

  Induction step: We will show that, for $r > 1$, $V^*_{i,r} = V'_{i,r}$.
  By inductive hypothesis we have $V^*_{i,r - 1} = V'_{i,r - 1}$.

  Let $\Z[j][i][r]$ denote the state of the emulated machine $\Z[j][i]$
  after simulation round $r$ (and note by Lemma~\ref{lem:in-vitro-independence}
  that this value does not depend on the \emph{in vitro} round or the round
  during which the \emph{for} loop is observed).

  Define the following machine configurations:

  \begin{enumerate}[label=(\Alph*)]
    \item The configuration $\Z[j][i][r - 1]$.
          \label{conj:simulation:config-A}
    \item The configuration of the machine $\PPi[i]$ at the end of round $r - 1$.
          \label{conj:simulation:config-B}
    \item The configuration $\Z[j][i][r]$.
          \label{conj:simulation:config-C}
    \item The configuration of the machine $\PPi[i]$ at the end of round $r$.
          \label{conj:simulation:config-D}
  \end{enumerate}

  By inductive hypothesis, configurations \ref{conj:simulation:config-A}
  and \ref{conj:simulation:config-B} are identical
  (here, we interpret $\Z[j][i][r-1]$ using \emph{in vitro} round $r_2 = r + \Delta_v$,
  \emph{simulation round} $r_3 = r - 1$, and \emph{for loop round} $r_1 = r - 1$ with reference to
  Lemma~\ref{lem:in-vitro-independence} notation).

  By the \rollerblade construction, % TODO: when does the construction obtain \writeboxes, \inboxes? In vitro r + \Delta_v + 1...
  the machine configuration~\ref{conj:simulation:config-C}
  $\Z[j][i][r]$ is the result of
  taking the machine configuration~\ref{conj:simulation:config-A}
  $\Z[j][i][r - 1]$
  (here, we interpret $\Z[j][i][r-1]$ using \emph{in vitro} round $R$,
  \emph{simulation round} $r_3 = r$, and \emph{for loop} round $r_2 = r - 1$)
  and running the \emph{for} loop for one iteration
  with inputs $\writeboxes[r - 1], \inboxes[r - 1]$, where
  $\writeboxes, \inboxes$ refers to the pair returned by $\prepareSimulationInputs$.
  ...
  to round $r$ by feeding it the inputs $\writeboxes[r - 1], \inboxes[r - 1]$.

  Because the execution $\mathcal{E}^*$ is consistent and the ledger is sticky,
  the \emph{in vitro} round and simulation round do not affect the output of $\prepareSimulationInputs$,
  similarly to the argument in the proof of Lemma~\ref{lem:in-vitro-independence},
  therefore $(\writeboxes, \inboxes) = \prepareSimulationInputs$ will be the same regardless
  of when they are observed. % TODO: explain in more detail what is meant here

  We conclude that $V^*_{i,r} = V'_{i,r}$.
  ...
  within the duration of $\mathcal{E}'$).
  \qed
