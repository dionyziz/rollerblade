...
\begin{proof}[Sketch]
  ...
  $\simulationRound = r_2$, and $r_3 \geq r_2$.
  ...
  (all transactions with recorded round $r - 1$ of $\Ledger[j][i][r + \Delta_v + 1]$
   ...
   appear in $\Ledger[j][i][r + \Delta_v + 1]$ by timeliness).
  Hence,
  \begin{align}
  ...
  and, since $\Pi$ is deterministic (because it is a \rollerblade-suitable-overlay protocol),
