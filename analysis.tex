...
\begin{proof}[Sketch]
  ...
  during the $r_3^\text{st}$ iteration of the \emph{for loop} in
  ...
  where $r_1 \geq r_2 + \Delta_v + 1$ with parameter
  $\simulationRound = r_2$, where $r_2 \geq r_3$.
  That this causes an invocation of \simulate with parameters
  ...
  \end{equation}
  and we wish to show that $c^*(r + \Delta_v + 1, r, r) = c'(r)$.

  Because $\mathcal{E}^*$ is consistent, we have that
  \begin{equation}\label{conj:simulation:consistency}
    c^*(r + \Delta_v + 1, r - 1, r - 1) = c^*(R, r - 1, r - 1)
  \end{equation}
  Each iteration of the \emph{for} loop of Algorithm~\ref{alg.simulate:for}
  ...
  be the pair
  returned by the invocation of $\prepareSimulationInputs(i, r, r + \Delta_v + 1)$
  \emph{in vitro} after round $r + \Delta_v + 1$ of $\mathcal{E}^*$, and, likewise define
  $(\writeboxes', \inboxes')$ to be the pair
  returned by the invocation of $\prepareSimulationInputs(i, R - \Delta_v - 1, R)$
  \emph{in vitro} after round $R$ of $\mathcal{E}^*$.
  ...
  and, since $\Pi$ is deterministic (because it is \rollerblade-overlay-respecting),
  therefore $c^*(r + \Delta_v + 1, r, r) = c'(r)$.
