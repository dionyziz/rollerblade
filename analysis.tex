...
\begin{proof}[Sketch]
  ...
  Let $(\writeboxes^{r-1}, \inboxes^{r-1})$ be the pair
  returned by the invocation of $\prepareSimulationInputs(i, r - 1, R)$
  \emph{in vitro} after round $R$, and, likewise define
  $(\writeboxes^r, \inboxes^r)$ to be the pair returned by the invocation
  of $\prepareSimulationInputs(i, r, R)$ \emph{in vitro} after round $R$.
  Because $\Y[j][i]$ is sticky, therefore $\Ledger[j][i][r - 1] \preceq \Ledger[j][i][r]$.
  Hence, $\writeboxes^{r - 1} \preceq \writeboxes^r$ and $\inboxes^{r - 1} \preceq \inboxes^r$
  ...
  and including round $r - 1$.

  Define the following machine configurations:

  \begin{enumerate}[label=(\Alph*)]
    \item The configuration of the machine $\Z[j][i]$ after the invocation of
          Algorithm~\ref{alg.simulate} Line~\ref{alg.simulate:execute}
          during the $(r - 1)$-st iteration of the \emph{for loop} in
          Algorithm~\ref{alg.simulate} Line~\ref{alg.simulate:for}
          when $\simulate$ is
          invoked \emph{in vitro} after round $R$ with parameter
          simulation round $r - 1$.
    ...
    \item The configuration of the machine $\Z[j][i]$ after the invocation of
          ...
          Algorithm~\ref{alg.simulate} Line~\ref{alg.simulate:for}
          when $\simulate$ is
          invoked \emph{in vitro} after round $r + \Delta_v$ with parameter
          simulation round $r - 1$.
    ...
    \item The configuration of the machine $\Z[j][i]$ after the invocation of
          ...
          Algorithm~\ref{alg.simulate} Line~\ref{alg.simulate:for}
          when $\simulate$ is
          invoked \emph{in vitro} after round $r + \Delta_v + 1$ with parameter
          simulation round $r$.
          \label{conj:simulation:config-D}
    \item The configuration of the machine $\PPi[i]$ at the end of round $r - 1$.
          \label{conj:simulation:config-E}
    \item The configuration of the machine $\PPi[i]$ at the end of round $r$.
          \label{conj:simulation:config-F}
  \end{enumerate}

  Observe that the state of the machine at each $(r - 1)$-st iteration of the
  \emph{for} loop only depends on the writeboxes and inboxes at position $(r - 1)$ and behind,
  namely $\writeboxes^{r - 1}[{:}{r - 1}], \inboxes^{r - 1}[{:}{r - 1}]$
  and $\writeboxes^r[{:}{r - 1}], \inboxes^r[{:}{r - 1}]$ respectively.
  Therefore, the configurations~\ref{conj:simulation:config-A} and~\ref{conj:simulation:config-B}
  are identical.

  Configurations \ref{conj:simulation:config-C} and \ref{conj:simulation:config-E} are
  identical because of the inductive hypothesis.

  Because $\mathcal{E}^*$ is consistent, therefore \ref{conj:simulation:config-A} and
  \ref{conj:simulation:config-C} are identical. Hence \ref{conj:simulation:config-A} and
  \ref{conj:simulation:config-E} are identical.

  \noindent
  \textbf{Subclaim 1b:}
  Consider the invocation of $\LLambda[j][i].\emulateMachine(i, r)$ \emph{in vitro}
  at the end of round $r + \Delta_v + 1$ of $\mathcal{E}^*$.
  This function emulated the execution of $\Z[j][i]$ in simulation rounds
  $1, \ldots, r$. Consider the inputs (writes and netins) $I^*$ provided
  to this $\Z[j][i]$ during simulation round $r$ in particular.
  Let $I'$ denote the inputs (writes and netints) provided to
  $\Pi^i$ at round $r$ in $\mathcal{E}'$. Then $I^* = I'$.

  {\color{red} Finish the claim proof...}

  We conclude that $V^*_{i,r} = V'_{i,r}$.

  {\color{red} Why does this conclude the proof of Claim 1?}
