...
\begin{definition}[Distributed Protocol Composition Faithfulness]
  A DPCP family
  $\Lambda_{\Pi,(\Y[][1], \ldots, \Y[][n])}$
  is \emph{faithful}
  for a family of overlay protocols $\mathcal{P}$ and a family of
  underlying protocols $\mathcal{Y}$ and a family of efficiently computable
  predicates $\mathcal{Q} = \{Q_Y\}_{Y \in \mathcal{Y}}$ of
  underlying protocol execution \emph{goodness}
  and a delay $\Delta(\cdot) \in \mathbb{N}$ if:
  for any overlay protocol $\Pi \in \mathcal{P}$
  and for any number of overlay parties $n \in \mathbb{N}$,
  ...
  $\PEExec(\Pi, \mathcal{S}, \mathcal{Z}', n, \mathcal{H}, \Delta((\Y[][1], \ldots, \Y[][n])))$,
  where the sampling of $\mathcal{E}$ is conditioned on the requirement that for all $i \in \mathcal{H}$
  the execution of $\Y[][i]$ satifies goodness $Q_{\Y[][i]}$ within $\mathcal{E}$.
\end{definition}
