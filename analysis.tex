...
Let $\LCExec[\Lambda][\Pi][({\Y[][1]}, \cdots, {\Y[][n]})][\mathcal{A}][\mathcal{Z}][n][\mathcal{H}]$
denote the transcript of an execution of the \rollerblade protocol with overlay protocol $\Pi$,
underlying protocols $\Y[][1], \ldots, \Y[][n]$, adversary $\mathcal{A}$, environment $\mathcal{Z}$,
number of parties $n$, indices of honest parties $\mathcal{H}$.
Following the notation of the UC framework
...
% TODO: Define the Ledger Composition problem and decompose it from the Rollerblade construction

\begin{definition}[Ledger Composability Faithfulness]
  A Ledger Composition protocol $\Lambda$ is \emph{faithful}
  for a set of overlay protocols $\mathcal{P}$ and a set of
  underlying protocols $\mathcal{Y}$ if:
  for any overlay protocol $\Pi \in \mathcal{P}$
  if for any number of overlay parties $n \in \mathbb{N}$,
  and any $n$ underlying protocols $\Y[][1], \ldots, \Y[][n] \in \mathcal{Y}$
  % TODO: allow "good" to be parametrized
  ...
  $\LCExec[\Lambda][\Pi][({\Y[][1]}, \cdots, {\Y[][n]})][n][\mathcal{H}][\mathcal{A}][\mathcal{Z}]$
  conditioned on the requirement that $\forall i \in \mathcal{H}: Y[][i] \text{ is \emph{good}}$
  and execution $\mathcal{E}'$ is sampled from
  $\PEExec[\Pi][\mathcal{S}][\mathcal{Z}'][n][\mathcal{H}]$.
\end{definition}
...
\begin{conjecture}[Simulation]\label{conj:simulation}
  The \rollerblade construction $\Lambda$ is \emph{faithful} for
  overlay protocols $\mathcal{P}$ that are \rollerblade-overlay,
  underlying protocols $\mathcal{Y}$ that are \rollerblade-underlying,
