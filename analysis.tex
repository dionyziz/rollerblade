...
\begin{definition}[Externality Similarity]
  Consider the party setting externalities $\PEExtern(V)$ and the emulated setting
  externalities $\LCExtern_{j,H}(\mathcal{E})$ for some party setting view $V$
  of honest parties, and some emulated setting execution $\mathcal{E}$.
  We say that the two externalities are \emph{similar with lateness parameter}
  $\Delta_u \in \mathbb{N}$, written
  $\PEExtern(V) \late{\Delta_u} \LCExtern_{j,H}(\mathcal{E})$, if
  the following holds.

  For any message $m$ located at position $(i, r)$ of $\LCExtern_{j,H}(\mathcal{E})$,
  there exists a round $r \leq r' \leq r + \Delta_u$ such that
  the message $m$ appears at position $(i, r')$ of $\PEExtern(V)$.
\end{definition}
