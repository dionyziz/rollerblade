...
\begin{definition}[Rollerblade Belief System and Honesty Correspondence]
  ...
  $
  \text{good}_i(\mathcal{E}) = \text{``} \Y[][i]
  $
  is safe, live$(u_i)$, and timely$(v_i)$ in $\mathcal{E}$''.
...
%  up to $r^* - v_i - u_{i'} - 1$ in $f^*$ and $\Ledger[j][i][R]$ are the same.
  Consider the call $\LLambda[j].\prepareEmulationInputs(\allowbreak i', \Ledger[j][i'][\hat{r}], \hat{r} - \Delta_v - 1)$.
...
\begin{proof}
  ...
  $\LLambda[j].\prepareEmulationInputs(i', \Ledger[j][i'][R - \Delta_v - 1], \allowbreak R - \Delta_v - 1)$.
  ...
  Hence, the $\msg$ must be included in $\inboxes[r^*]$ after $\LLambda[j].\prepareEmulationInputs(\allowbreak i', \Ledger[j][i'][R], R - \Delta_v - 1)$
  ...
  $\{\textsf{to}: i', \msg: \msg\} \in \LLambda[j].\emulate(i, F^i, \allowbreak r^* - v_i - u_{i'} - 1).\outboxes[r]$,
